class MaxHeap {
    ...
    delete() {
        ...
        // 1: swap the root/max/1st element to last element
        this.swap(0, this.heap.length - 1);

        // 2: remove the last element from heap --> i.e. is the root
        this.heap.pop();

        // 3: arrange the swapped/1st element to its correct place
        let index = 0;
        ...
        while (true) {
            ...
            if (leftIndex < size && this.heap[largest] < this.heap[leftIndex]) {
                largest = leftIndex;
            }
...
class MinHeap {
    ...
    // delete the min / root element
    delete() {
        // edge case
        if (this.heap.length == 0) return null;

        // 1: swap the root/min/1st element to last element
        this.swap(0, this.heap.length - 1);
...
/**
 *
 * deletion in heap happens in O(log N) Time complexity at worst or avg case
 * best case when the element are in correct place then O(1)
 *
 */
